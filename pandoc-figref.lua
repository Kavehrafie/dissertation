...
function references_generator(word)
    if table_contains(figref_table, word) then
	    ...
	    -- For docx, we'll just return the number as plain text
	    return pandoc.Str(referenceNumber)
        end
    else
        return nil
    end
...
function figure_parser(figure)
    ...
    -- Add a caption with figure number
    if not figure.caption.long[1] then
        figure.caption.long = pandoc.List({pandoc.Plain({})})
    end
    figure.caption.long[1].content:insert(1, pandoc.Str("Figure " .. counter .. ": "))
end
